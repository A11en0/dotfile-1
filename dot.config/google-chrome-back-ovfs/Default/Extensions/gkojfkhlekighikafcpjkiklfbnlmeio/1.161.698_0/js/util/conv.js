...
function apply_scale(num, opt){
    opt = opt||{};
    var per = opt.per, scale = opt.scale;
    ...
    return {num: num, per: per, base: base, scale: scale_vals[i]};
}

E.get_scale = function(num, opt){ return apply_scale(num, opt).scale; };
...
E.scaled_number = function(num, opt){
    ...
    var _per = function(){ return res.per ? E.fmt_per(res.per) : ''; };
    num = res.num;
    if (opt.is_scale)
        return res.scale.n;
    num /= res.scale.n;
    ...
    return sign+str.replace(/((\.\d*[1-9])|\.)0*$/, '$2')
        +(units ? (opt.space ? ' ' : '')+res.scale.s : '')+_per();
};
