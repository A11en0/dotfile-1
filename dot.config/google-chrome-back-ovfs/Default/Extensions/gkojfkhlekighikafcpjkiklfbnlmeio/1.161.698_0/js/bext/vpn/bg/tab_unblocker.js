...
define(['underscore', '/bext/pub/backbone.js',
    ...
    '/util/escape.js', '/util/url.js', '/util/date.js',
    ...
    B, svc_util, zescape, zurl, date, zerr, browser,
...
function nodify_res(details){
    ...
    if (!n)
    {
        be_util.perr('bad_status_line', {statusLine: details.statusLine,
            url: details.url, method: details.method,
            statusCode: details.statusCode}, {rate_limit: {count: 2}});
...
class Bw_perr_acc {
    ...
    _send(bw_rule, acc){
        acc.id = bw_rule.id;
        be_util.perr('req_bw', {len: acc.len, ms: acc.ms,
            bw: acc.len*1000/acc.ms, id: acc.id,
            country: acc.rule && acc.rule.country});
...
function on_hooks_err(id, err){
    be_util.perr(id, {}, {err, rate_limit: {count: 1}}); }
...
class Base_handler {
    constructor(){
        ...
        this.on_completed_debug = ff_cb_wrapper('on_completed_debug', d=>{
            ...
            {
                be_util.perr('bw_req_err_'+verify.id,
                    assign({error: req.proxy_resp.code}, info),
                    {rate_limit: limit});
            }
            ...
            if (tab_unblocker && req.proxy_req && d.ip &&
                ...
                !d.fromCache)
            {
                be_util.perr('req_agent_ip_mismatch', info,
                    {rate_limit: {count: 1}});
    ...
    tab_unblocker_cb(details){
        ...
        if (!be_ext.get('agent_key') && !storage.get('agent_key'))
        {
            unblocker_lib.on_before(url, details.requestId, details.tabId,
                {err: 'agent_key'});
            be_util.perr('be_no_agent_key2', {}, {err: true,
                rate_limit: {count: 2}});
            return; 
        }
        let r;
    ...
    _on_headers_received(details){
        ...
        if (req.proxy_req)
        {
            if (hola_agent) 
            {
                ...
                return void be_util.perr('req_header_proxy_auth',
                    {url: req.url, proxy_req: req.proxy_req},
                    {err: true, rate_limit: {count: 5}});
            }
            req.proxy_resp = resp;
            if (!req.direct_resp)
        ...
        if (cmd.proxy)
        {
            if (cmd.proxy.serve)
            {
                ...
                {
                    be_util.perr('req_invalid_redirect', {url: req.url,
    ...
    _unsafe_tab_unblocker_end_cb(details){
        ...
        if (!errors.includes(details.error))
        {
            ...
            return void be_util.perr('req_err_unhandled', info,
                {err: details.error, rate_limit: {count: 5}});
        }
...
function is_vpn_allowed(_url, is_main, details){
    ...
    {
        if (protocol!='file:')
            be_util.perr('url_parsing_failed', {url});
        return false;
    }
...
function get_proxy_agent(req, proxy){
    ...
    if (!zutil.get(agents, 'length'))
    {
        ...
        be_util.perr('be_debug_no_agents', zerr.json({route_opt: req.route_opt,
            agents: unblocker_lib.get_active_agents(rule),
            rule: be_vpn_util.get_rule_min_fmt(rule)}), {err: true});
        return;
    }
    ...
    if (agent.vendor!='force_agent' && req.route_opt.pool && !agent.pool)
    {
        be_util.perr('be_wrong_agent', {agent,
...
function tab_unblocker_del(tabid, refresh){
    ...
    if (!rule_info)
    {
        be_util.perr('be_rule_info_missing', {tabid, tab_unblocker,
            rule_infos: E.url_to_rule_infos});
    }
    else
...
E.update_rule_urls = rules=>etask(function*update_rule_urls(){
    ...
    if (be_dev_mode.get('debug_pac')) 
        pac_engine.init({rule_cleanup_ms: bext_config.pac_rule_cleanup_ms});
    let ag_rules = [], agents = {};
    ...
    yield unblocker_lib.resolve_agents(ag_rules, null, null,
        'update_rule_urls');
...
const on_tab_completed = cb_wrapper(o=>{
    ...
    if (tab && (rule = tab.rule))
        ...
        info.local = true;
    be_util.perr('tab_load_completed', info);
});
...
function init_mitm(){
    if (mitm_lib.inited || !mitm_lib.init({ajax_via_proxy: E.ajax_via_proxy,
        storage, perr: be_util.perr, loc: be_info.get('location'), zerr,
...
E.send_not_working = (tab, reason)=>{
    ...
    be_util.perr('be_not_working_stats', assign({reason}, tab.not_working),
        {err: true, with_log, rate_limit: {count: 5, ms: date.ms.MIN}});
...
function on_not_working(e){
    ...
    {
        return void be_util.perr('be_not_working_tab_not_found',
            e, {err: true, rate_limit: {count: 1}});
    }
...
function get_credentials(){
    ...
    if (err)
        be_util.perr(err, {}, {err: true, rate_limit: {count: 2}});
...
const agent_auth_listener = ff_cb_wrapper('agent_auth_listener', details=>{
    ...
    be_util.perr('auth_listener_not_proxy', _.pick(details, 'url', 'realm',
        'challenger'), {rate_limit: {count: 2}});
    return {};
});
...
const on_main_frame_headers = d=>{
    ...
    {
        be_util.perr('browse_failure_detected', {url: d.url,
            code: d.statusCode, page_url: tab_url,
