...
E.debug.bug_report = (tab_id, info)=>{
    if (isNaN(tab_id))
        info = tab_id;
    info = assign({uuid: api.uuid,
        proxy_country: 'us'}, E.debug.get_debug_info(tab_id), info);
    if (!zutil.is_mocha())
        perr('mitm_bug_report', info, {filehead: zerr.json(last_log)});
};
