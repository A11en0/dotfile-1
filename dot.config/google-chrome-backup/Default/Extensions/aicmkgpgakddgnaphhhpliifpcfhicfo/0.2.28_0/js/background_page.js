...
/**
 * @description it is used to log the errors/messages in background page
 * and sends the same to the popup if the port is connected
 */
const interceptor = {
    log: function (msg) {
      console.log(msg);
...
// not using this method, might use in future
...
function setBlueIcon () {
  chrome.browserAction.setIcon({
    path: '../assets/interceptor_48x48_blue.png'
  });
}
...
function setOrangeIcon () {
  chrome.browserAction.setIcon({
    path: '../assets/interceptor_48x48.png'
  });
}
...
function sendErrorToPostman (postmanMessage, error) {
  var guid = postmanMessage.guid,
    customAppId = postmanMessage.postmanAppId;
  if (!customAppId) {
    ...
    if (thisCookie.length >= 1) {
      ...
      try {
        var cName = thisCookie.splice(0, 1)[0], // this is the part before the first =
          cValue = thisCookie.join('='); // part after the first =
        chrome.cookies.set({
          url: url,
          name: cName,
          value: cValue
...
function sendXhrRequest (postmanMessage) {
  ...
  for (i = 0, len = headers.length; i < len; i++) {
    ...
    if (upperCasedHeader === 'COOKIE') {
      ...
      else if (upperCasedHeader.startsWith('PROXY-') || upperCasedHeader.startsWith('SEC-')) {
        headers[i].name = 'Postman-' + headers[i].name;
      }
    }
  }
  ...
  var url = currentRequest.url,
    dataMode = currentRequest.dataMode,
    xhrTimeout = currentRequest.xhrTimeout,
  ...
  xhr.onload = (function (xhr, postmanMessage, xPostmanInterceptorId) {
    return function () {
      ...
      var response,
        ...
        unpackedHeaders = unpackHeaders(xhr.getAllResponseHeaders()),
...
function onBeforeSendHeaders (details) {
  var hasRestrictedHeader = _.find(details.requestHeaders, function (headerObject) {
      return headerObject.name.startsWith('Postman-');
    }),
 ...
 *
 * @param {object} request
 * @description filters requests based on URL, methods (received from Postman Native App)
 * before sending to Postman Native App
 ...
 *
 * @param {object} details
 * @description  for filtered requests it sets the headers on the request in requestcache.
 * It is fired after extension have had a chance to modify the request headers,
 * and presents the final version.
 */
...
function sendCapturedRequestToPostman (reqId, appType) {
  // eslint-disable-next-line no-undef
  var loggerMsg = '<span class="' + addClassForRequest(requestCache[reqId].method) + '">' +
    requestCache[reqId].method + '</span><span class="captured-request-url">' +
    (requestCache[reqId].url).substring(0, 150) + '</span>',

    request = requestCache[reqId],
    // eslint-disable-next-line no-undef
    methodWithBody = isMethodWithBody(request.method),
    requestBodyType;
...
/**
 *
 * @description Called when a port is disconnected. Restarts pinger
 */
...
/**
 *
 * @description this method is called when a port is connected
 * no need to ping this port anymore as the port is already connected
 */
...
/**
 *
 * @description attempts to re-ping when port does not exist
 */
    ...
    chrome.runtime.sendNativeMessage('com.postman.postmanapp', {
      ...
      if (!response) {
        ...
        if (nativeHostInstalled) {
          ...
          interceptor.log('Postman <> Interceptor Connectivity:' +
            'Please install the interceptor bridge to connect to the Postman app.');
        }
        return;
      }
 ...
 */
  sendRemovedCookiesOnce = _.debounce(sendRemovedCookiesToPostman, 2000);


/**
 *
 * @param {object} cookie
 * @description it checks if the changed cookie already exists in updatedCookies array
 ...
 *
 * @param {object} cookie
 * @description it checks for the cookie passed is whether present or not in removedCookies array.
 * It returns index if present else -1
 */
...
/**
 *
 * @param {cookie} obj this contains the change info
 * i.e. underlying cause for cookie change, removed (true/false) and cookie object
 * @description listener for chrome.cookies.onChanged event.
 * It's called whenever there's any change in the browser cookies.
 */
function cookieChangeListener (obj) {
  // perform actions only when domain list is not empty & enabled is true,
  // otherwise it's not required to send updates to native interceptor bridge
...
/**
 * @description it fetches the cookie sync options from local storage i.e. syncEnabled (True/False) and syncDomainList,
 * updates it to global and attaches the listeners
 */
...
function removeDomainFromSyncDomainList (domain) {
  ...
  var domainList = appCookieSyncOptions.syncDomainList,
    updatedDomainList = domainList.filter(function (value, index, arr) {
      return value !== domain;
    });
...
/**
 * @description it sets the secret key to the local storage
 * and stores it as global which is used for encryption/decryption
 */
...
/**
* @description it fetches the secret key from local storage
* and stores it as global which is used for encryption/decryption
* if secret key is not set in local storage,
* it sets the default key i.e. 'postman_default_key' in chrome's local storage
*/
...
function sendMessageToNativeHost (message) {
  if (nativePort) {
    try {
      ...
      interceptor.log('Postman <> Interceptor Connectivity:' +
        'Error occurred while sending a message to the native host' + err.toString());
    }
  }
  else {
