// LICENSE_CODE ZON
'use strict'; 
define(['underscore', '/util/zerr.js', '/util/version_util.js',
    '/util/etask.js', '/util/date.js', '/util/escape.js', '/util/sprintf.js',
    '/util/user_agent.js', '/util/ajax.js', '/util/storage.js',
    '/util/array.js', '/util/util.js', '/bext/vpn/util/util.js',
    '/svc/vpn/pub/util.js', 'zon_config', 'conf'],
    function(_, zerr, version_util, etask, date, zescape, sprintf, user_agent,
    ajax, storage, array, zutil, be_vpn_util, svc_util, zconf, conf)
{
const E = {};
...
E.build_info = opt=>{
    ...
    if (chrome.runtime && chrome.runtime.getManifest)
        info.manifest_version = chrome.runtime.getManifest().version;
    if (mode)
    {
...
E.perr_opt = (id, info, opt)=>{
    ...
    let {err, bt, filehead} = opt, ver = E.version();
    bt = err ? zerr.e2s(err) : bt;
    ...
    let qs = {id, ext_ver: ver, product: E.get_product(),
        browser: E.browser()};
    if (opt.with_log)
    ...
    opt.level = opt.level || (err||bt||id.match(err_re) ? zerr.L.ERR :
        zerr.L.NOTICE);
    return {id, info, opt};
};
...
E.perr = (id, info, opt, new_name)=>etask(function*(){
    info = info||{};
    opt = opt||{};
    const bg_main = get_bg_module('be_bg_main');
    id = E.perr_id(id, new_name);
    if (conf.check_agree_ts && !(bg_main && bg_main.get('agree_ts')))
        return void zerr('no agree_ts, skip perr %s', id);
    let sample;
    if (sample = E.CG(`perr_sample.${id}`))
    {
        if (Math.random()>=sample)
            return;
        id = id+'_sample';
        info = assign({}, info, {sample});
    }
