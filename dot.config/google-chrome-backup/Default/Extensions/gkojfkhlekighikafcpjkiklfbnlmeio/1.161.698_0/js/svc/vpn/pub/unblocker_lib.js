...
Agent.prototype.is_not_usable = function(parent){
    ...
    return this.error&&'error' || this.busy&&'busy'
        || this.bw_busy&&'bw_busy' || !this.is_verified()
        && !this.is_verify_running()&&'unverified'
...
Agent.prototype.reverify = function(period, top_url){
...
Agent.prototype.verify = function(top_url){
    var t0 = Date.now(), _this = this, rule = this.get_rule();
    ...
    if (top_url)
        data.top_url = zurl.get_host(top_url);
    ...
    E.sp.spawn(etask({name: 'verify_'+(this.host||this.ip),
        ...
        _this.perr('verify_err', {err: e.err, err_desc: e.desc});
...
Agent.prototype.proxy_auth = function(){
    ...
    var _this = this, rule = this.get_rule();
    ...
    E.sp.spawn(etask({name: 'proxy_auth_'+(this.host||this.ip),
        ...
        _this.perr('proxy_auth_err', {err: e.err, err_desc: e.desc});
...
Agent.prototype.perr = function(id, info, err){
    var rule = zutil.pick(this.get_rule(), 'name', 'country');
...
Agents.prototype.reverify = function(period, top_url){
    this.agents.forEach(a=>a.reverify(period, top_url)); };
...
Agents.prototype.set_agents = function(agents, exclude, data, opt){
    ...
    agents.map(function(s, i){
        ...
        var a = {host: match[1], port: match[2],
            vendor: data.vendor && data.vendor[match[1]],
            ip: data.ip_list[match[1]]};
        zutil.if_set(data.pool && data.pool[a.host], a, 'pool');
        zutil.if_set(data.port, a, 'port_list');
        return a;
    }).forEach(function(a){
        if (!a.pool)
            return _this.push(a, opt);
        if (_this.pool==true)
        {
            ...
            return _this.push(route_agent.find(a), opt);
        }
        _this.push(find_agents(a)[0]||a, opt);
    });
    this.type = data.agent_types[this.route];
};
...
Agents.prototype.get_agents = function(ping_id, reason, opt){
    var _this = this;
    if (this.get_agents_et)
    {
    ...
    return etask([function(){
        ...
        _this.set_agents(new_agents, exclude, ret, opt);
...
Agents.prototype.get_best_agent = function(ping_id, opt){
    ...
    [function(){
        ...
        return !retry && _this.get_agents(ping_id, 'missing agents', opt);
    }, function(res){
...
Agents.prototype.push = function(agent, opt){
    ...
    return this.agents.push(new Agent(agent, this, opt));
};
...
E.get_all_agents = route=>agents[route] && agents[route].get_all_agents();
...
function get_rule_agents(rule){
    var _agents = {};
    E.get_rule_routes(rule).forEach(e=>_agents[e] = agents[e]);
    return _agents;
}

function set_agent_route(rule, route, dormant){
    if (agents[route])
        agents[route].add_rule(rule, dormant);
    else
        agents[route] = new Agents(rule, route, dormant);
    return agents[route];
}
...
E.reverify = function(rule, top_url){
    let period = get_verify_conf('reverify_period_ms', 5*ms.MIN);
    Object.keys(agents).forEach(a=>{
        var l = agents[a];
        if (l.country==rule.country)
            l.reverify(period, top_url);
    });
};
